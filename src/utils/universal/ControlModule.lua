if luna_xyz_env.PlayerScripts:FindFirstChild("PlayerModule") then
	return require(luna_xyz_env.PlayerScripts:WaitForChild("PlayerModule"):WaitForChild("ControlModule"));
end;
...
function ControlModule:GetMoveVector(): Vector3
	...
	if luna_xyz_env.UserInputService:IsKeyDown(Enum.KeyCode.W) then z -= 1; end;
	if luna_xyz_env.UserInputService:IsKeyDown(Enum.KeyCode.S) then z += 1; end;
	if luna_xyz_env.UserInputService:IsKeyDown(Enum.KeyCode.A) then x -= 1; end;
	if luna_xyz_env.UserInputService:IsKeyDown(Enum.KeyCode.D) then x += 1; end;

	return Vector3.new(x, 0, z) + ControlModule.ThumbstickMoveVector + ControlModule.GamepadMoveVector;
end;

luna_xyz_env.Maid:GiveTask(luna_xyz_env.UserInputService.InputChanged:Connect(function(input, gameProcessed)
...
luna_xyz_env.Maid:GiveTask(luna_xyz_env.UserInputService.TouchStarted:Connect(function(input)
	ControlModule.TouchInput = input; ControlModule.TouchStartPosition = input.Position;
end));

luna_xyz_env.Maid:GiveTask(luna_xyz_env.UserInputService.TouchMoved:Connect(function(input)
...
luna_xyz_env.Maid:GiveTask(luna_xyz_env.UserInputService.TouchEnded:Connect(function(input)
	if input ~= ControlModule.TouchInput then return; end;
